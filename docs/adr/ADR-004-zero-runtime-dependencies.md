...
upstream), making the baseline flaky.

## Decision

The application uses only the Node standard library — zero runtime dependencies. The
insecure scenario is exercised deliberately as a regression check: a PR adds a dependency
with a known CVE (e.g. `lodash@4.17.19`) or hardcodes a secret, and the gate blocks it.
